    ...
    this.progressMatrixService.getActiveMatrix(type).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 403) {
            ...
            const forbiddenMessage = err.error?.message || 'No tiene permisos para ver esta matriz.';
            ...
            this.notificationService.showError('Error de API', 'No se pudo cargar la configuración de la matriz activa.');
          }
      },
    });
  }
